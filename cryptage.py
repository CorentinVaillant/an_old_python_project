...
cle = [17,42,254] #[a(compris entre 2 et 20), b , c]   !!cle n'est pas une constante mais pratique pour l'exercice !!
...
def unCrypt(lettres, clee):
    for l in range(len(lettres)):
        ...
        lettres[l] = lettres[l] - cle[1] - cle[2]*(cle[0]**l)   #soustraction de la clée2 (plus clée 3 à chaque lettre)
...
def motALaBonneTaille(phrase):
    if type(phrase) == str: li = False
    elif type(phrase) == list: li = True
    else : print('error, not a list or a string !'); quit()
    mots = []
    phrase = list(phrase)
    while len(phrase)> 20:
        if li :mots.append((phrase[:21]))
        elif li != True : mots.append(''.join(phrase[:21]))
        del phrase[:21]
...
def cryptLong(mot,cle):
    ...
    return (phrase)
def unCryptLong(lettres,cle):
    #lettres = motALaBonneTaille(lettres)
    phrase = []
    for k in lettres:
